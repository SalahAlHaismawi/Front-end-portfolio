          ...
          <Slider {...cardSettings} className="w-full h-full">
            {projects.map((project, index) => (
                <div key={index} className="w-full h-full">
                  ...
                  <div
                      className="bg-[#1F2833] h-full rounded-xl p-6 flex flex-col justify-between items-center shadow-lg">
                    <h3 className="text-2xl font-bold text-[#66fcf1] mb-4">{project.name}</h3>
                    <Slider {...imageSettings} className="w-full h-full mb-4">
                      {project.images && project.images.length > 0 ? project.images.map((image, imgIndex) => (
                          <div key={imgIndex}
                            ...
                            <img src={image} alt={`Project ${index + 1} Image ${imgIndex + 1}`}
                                 className="rounded-xl object-contain w-full h-full max-w-[800px] mx-auto"/>
                          </div>
